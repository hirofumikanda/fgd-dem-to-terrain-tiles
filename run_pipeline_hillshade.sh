...
# Hillshadeパイプラインスクリプト - DEM10BからHillshade PMTilesまでの全工程を実行
# 作成日: $(date +%Y-%m-%d)

# 変数設定
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
LOG_DIR="./logs"
MASTER_LOG_FILE="$LOG_DIR/master_hillshade_$(date +%Y%m%d_%H%M%S).log"
...
pre_execution_check() {
    ...
    if [ $start_step -eq 6 ]; then
        if [ ! -f "./dem_3857_hillshade.tif" ]; then
            master_log "❌ エラー: ./dem_3857_hillshade.tif が見つかりません（ステップ1-5が未実行）"
            exit 1
        fi
        master_log "✅ Hillshadeファイルを確認: ./dem_3857_hillshade.tif"
    fi
...
execute_pipeline() {
    ...
    master_log "🚀 DEM to Hillshade タイル配信形式 パイプライン開始"
    ...
    if [ $failed_scripts -eq 0 ]; then
        ...
        [ -f "./dem_3857_hillshade.tif" ] && master_log "   Hillshade: ./dem_3857_hillshade.tif"
        [ -d "./tiles_hillshade" ] && master_log "   ラスタタイル: ./tiles_hillshade ($(find ./tiles_hillshade -name "*.png" 2>/dev/null | wc -l) タイル)"
        [ -f "./dem_3857_hillshade.mbtiles" ] && master_log "   MBTiles: ./dem_3857_hillshade.mbtiles"
        [ -f "./dem_3857_hillshade.pmtiles" ] && master_log "   PMTiles: ./dem_3857_hillshade.pmtiles"
        
        master_log ""
        master_log "✨ Hillshadeパイプライン正常完了 ✨"
        exit 0
    else
...
show_usage() {
    ...
    echo "  5: Hillshade生成"
    echo "  6: ラスタタイル・MBTiles・PMTiles作成"
    echo ""
    echo "例:"
    echo "  $0              # 全ステップを実行"
    ...
    echo ""
    echo "説明:"
    echo "  DEM10B ZIPファイルからHillshadeの複数配信形式（ラスタタイル・MBTiles・PMTiles）までの全工程を自動実行します"
