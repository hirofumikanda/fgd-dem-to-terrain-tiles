...
# 等高線パイプラインスクリプト - DEM10Bから等高線PMTilesまでの全工程を実行
# 作成日: $(date +%Y-%m-%d)

# 変数設定
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
LOG_DIR="./logs"
MASTER_LOG_FILE="$LOG_DIR/master_contour_$(date +%Y%m%d_%H%M%S).log"
...
pre_execution_check() {
    ...
    if [ $start_step -eq 6 ]; then
        if [ ! -f "./dem_3857_contour.geojson" ]; then
            master_log "❌ エラー: ./dem_3857_contour.geojson が見つかりません（ステップ1-5が未実行）"
            exit 1
        fi
        master_log "✅ 等高線GeoJSONファイルを確認: ./dem_3857_contour.geojson"
    fi
...
execute_pipeline() {
    ...
    master_log "🚀 DEM to 等高線 PMTiles パイプライン開始"
    ...
    if [ $failed_scripts -eq 0 ]; then
        ...
        # 等高線ファイルの詳細情報
        if [ -f "./dem_3857_contour.geojson" ]; then
            contour_size=$(du -h "./dem_3857_contour.geojson" | cut -f1)
            contour_features=$(wc -l < "./dem_3857_contour.geojson" 2>/dev/null || echo "N/A")
            master_log "   等高線GeoJSON: ./dem_3857_contour.geojson ($contour_size, $contour_features フィーチャー)"
        fi
        
        [ -f "./dem_3857_contour.mbtiles" ] && master_log "   等高線MBTiles: ./dem_3857_contour.mbtiles"
        [ -f "./dem_3857_contour.pmtiles" ] && master_log "   等高線PMTiles: ./dem_3857_contour.pmtiles"
        
        master_log ""
        master_log "✨ 等高線パイプライン正常完了 ✨"
        exit 0
    else
...
show_usage() {
    ...
    echo "  5: 等高線生成（10m間隔・GeoJSONSeq形式）"
    echo "  6: MVT/PMTilesタイル作成"
    echo ""
    echo "例:"
    echo "  $0              # 全ステップを実行"
    ...
    echo ""
    echo "説明:"
    echo "  DEM10B ZIPファイルから等高線PMTilesまでの全工程を自動実行します"
