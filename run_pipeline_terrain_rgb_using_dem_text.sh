...
# エントリスクリプト - DEM10BからDEMテキストタイル経由でTerrain RGBタイルまでの全工程を実行
# 作成日: $(date +%Y-%m-%d)

# 変数設定
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
LOG_DIR="./logs"
MASTER_LOG_FILE="$LOG_DIR/master_terrainrgb_dem_text_$(date +%Y%m%d_%H%M%S).log"
...
SCRIPTS=(
    ...
    "06_create_tiles_terrain_rgb_from_dem_text.sh"
)
...
pre_execution_check() {
    ...
    if [ $start_step -eq 6 ]; then
        if [ ! -d "./tiles_elevation" ] || [ $(find "./tiles_elevation" -name "*.txt" | wc -l) -eq 0 ]; then
            master_log "❌ エラー: DEMテキストタイルが見つかりません（ステップ1-5が未実行）"
            exit 1
        fi
        master_log "✅ DEMテキストタイルを確認: ./tiles_elevation"
    fi
    ...
    # ステップ5以降で必要なgenerate_text_tiles.py確認
    if [ $start_step -le 5 ] && [ ! -f "./generate_text_tiles.py" ]; then
        master_log "❌ エラー: generate_text_tiles.py が見つかりません"
        exit 1
    fi
    
    # ステップ6で必要なconvert_text_to_terrainrgb.py確認
    if [ $start_step -le 6 ] && [ ! -f "./convert_text_to_terrainrgb.py" ]; then
        master_log "❌ エラー: convert_text_to_terrainrgb.py が見つかりません"
        exit 1
    fi
...
execute_pipeline() {
    ...
    master_log "🚀 DEM to Terrain RGB Tiles (via DEM Text) パイプライン開始"
    ...
    if [ $failed_scripts -eq 0 ]; then
        ...
        [ -d "./tiles_elevation" ] && master_log "   DEMテキストタイル: ./tiles_elevation ($(find ./tiles_elevation -name "*.txt" | wc -l) ファイル)"
        [ -d "./tiles_terrainrgb" ] && master_log "   Terrain RGBタイル: ./tiles_terrainrgb ($(find ./tiles_terrainrgb -name "*.png" | wc -l) ファイル)"
        [ -f "./dem_3857_terrainrgb.mbtiles" ] && master_log "   MBTiles: ./dem_3857_terrainrgb.mbtiles"
        [ -f "./dem_3857_terrainrgb.pmtiles" ] && master_log "   PMTiles: ./dem_3857_terrainrgb.pmtiles"
        
        master_log ""
        master_log "✨ DEMテキストタイル経由パイプライン正常完了 ✨"
        exit 0
    else
...
show_usage() {
    ...
    echo "  5: DEMテキストタイル生成"
    echo "  6: DEMテキストからTerrain RGBタイル生成"
    echo ""
    echo "例:"
    echo "  $0              # 全ステップを実行"
    ...
    echo ""
    echo "説明:"
    echo "  DEM10B ZIPファイルからDEMテキストタイル経由でTerrain RGBタイルまでの全工程を自動実行します"
